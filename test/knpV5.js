const NetworkProxy = artifacts.require("./KyberNetworkProxyV5.sol");
...
const Network = artifacts.require("./KyberNetworkV5.sol");
...
//        console.log("current block: " + currentBlock);
    ...
    it("should set base rates + compact data rate factor + step function. for all tokens.", async function () {
        ...
        for (i = 0; i < numTokens; ++i) {
            ...
            baseBuyRate2.push(tokensPerEther.mul(10100).div(10000).floor().valueOf());
            baseSellRate1.push(ethersPerToken.valueOf());
            baseSellRate2.push(ethersPerToken.div(1000).mul(980).floor().valueOf());
        }
    ...
    //    console.log(baseSellRate2)
        ...
        let baseBuyUnique = [precisionUnits.mul(18).floor().valueOf()];
        let baseSellUnique = [precisionUnits.div(18).floor().valueOf()];
//        log(uniqueAddArr + "  " + baseBuyUnique + "  " + baseSellUnique)
        ...
        baseBuyUnique = [precisionUnits.mul(2).floor().valueOf()];
        baseSellUnique = [precisionUnits.div(2).floor().valueOf()];
//        log(uniqueAddArr + "  " + baseBuyUnique + "  " + baseSellUnique)
    ...
    it("should init network and reserves and set all reserve data including balances", async function () {
        ...
        for (let i = 0; i < numTokens; ++i) {
            ...
            if(i == 0) {
                await token.transfer(walletForToken, amount1.toFixed());
                await token.approve(reserve1.address,amount1.toFixed(),{from:walletForToken});
                await reserve1.setTokenWallet(token.address,walletForToken);
                balance = await token.balanceOf(walletForToken);
            }
            else {
                await token.transfer(reserve1.address, amount1.toFixed());
            ...
            await token.transfer(reserve2.address, amount2.toFixed());

            assert.equal(amount1.toFixed(), balance.valueOf());
...
//           log("txData logs 0" + txData.logs[0])
    ...
    it("use trade with hint. see hint size > 0 reverts", async function () {
        ...
        assert.equal(user2BalanceAfter.toNumber(), user2BalanceBefore.toNumber());
    });
    ...
    it("use trade with hint. disable 1 reserve. perform sell and check: balances changed as expected.", async function () {
        ...
        try {
            ...
            let txData = await networkProxy.tradeWithHint(tokenAdd[tokenInd], amountTwei, ethAddress, user2, 500000,
                            rate[1], walletId, hint, {from:user1});
...
//        log("buy rate 1: " + buyRate1 + " buyRate2 " + buyRate2 + " diff rate: " + (buyRate2 * 10000 / (10000 + negligibleDiff)) );
    ...
    it("should set reserve sell rate diff > negligibleDiff. perform sell and make sure done on expected reserve.", async function () {
        ...
        assert.equal(balance.valueOf(), expectedEthBalanceUser2.toFixed(), "bad balance user2.");
...
//        log("allowance " + allowance)
    ...
    it("should verify trade reverted when rate below min rate.", async function () {
        let tokenInd = 1;
        let token = tokens[tokenInd]; //choose some token
        let amountTWei = 1000*1;
        let maxDestAmt = 5000000;
        ...
        await networkProxy.trade(tokenAdd[tokenInd], amountTWei.valueOf(), ethAddress, user2, maxDestAmt,
        ...
        txData = await networkProxy.trade(tokenAdd[tokenInd], amountTWei.valueOf(), ethAddress, user2,
                maxDestAmt.valueOf(), 0, walletId, {from:user1});
    });
    ...
    it("should verify trade reverted when rate above max rate.", async function () {
        ...
        try {
            await networkProxy.trade(tokenAdd[tokenInd], amountTWei, ethAddress, user2, maxDestAmt, 0, walletId, {from:user1});
            assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected revert but got: " + e);
        }
        ...
        await networkProxy.trade(tokenAdd[tokenInd], amountTWei.valueOf(), ethAddress,
                user2, maxDestAmt, 0, walletId, {from:user1});
    });
